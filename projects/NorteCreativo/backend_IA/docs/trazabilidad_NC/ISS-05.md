...
**Commit esperado:** `feat(iss-05): feature hitos CA` con `Refs #__`

> El estado del issue **vive en el tablero Kanban**, no en este archivo.
> El Hito es la entidad con **estado** (ABIERTO/CERRADO/FACTURADO) que sostiene la regla central del proyecto (RN-01, RN-02, RN-06). Su cierre se implementa en ISS-07.
...
- [ ] domain (entidad con `cerrar()`, interface, excepciones)
- [ ] application (DTO, mapper, use-cases con RN-08)
- [ ] infrastructure (model con estado/fechaCierre, repo con `actualizarEstado`, seeder) + `ALL_MODELS`
- [ ] presentation (controller)
- [ ] módulo (importa CampaniasModule) en `BusinessModule`
...
|       | HTTP 201 estado ABIERTO | AC-2 | (respuesta) | `curl -i -X POST localhost:3011/api/hitos -d '{"campaniaId":1,"nombre":"Redes"}' -H 'Content-Type: application/json'` |
|       | HTTP 404 (FK) | AC-3 | (respuesta) | `curl` con `campaniaId:999999` |
|       | HTTP 400 | AC-4 | (respuesta) | `curl` sin `nombre` |
|       | HTTP 404 | AC-5 | (respuesta) | `curl -i localhost:3011/api/hitos/999999` |
|       | archivo fuente | AC-6 | `.../hito.entity.ts` | `rg -n "sequelize|@nestjs|extends Model" <ruta>` |

**Commit (hash):** pendiente — `feat(iss-05): feature hitos CA` · `Refs #__`
